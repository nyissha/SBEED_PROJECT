...
class Args:
    def __init__(self):
        # Environment & Data
        self.env = "Acrobot-v1"
        self.data_path = "acrobot/D_acrobot_medium_mixed.npz"
        self.seed = 1
        ...
        self.lam = 0.01
        self.gamma = 0.99
        self.kl_beta = 0.1 # KL penalty coefficient (inverted)
        self.grad_clip = 10.0
        
        # Visualization
        self.viz_mode = "compare" # choices: ["sbeed_only", "compare"]
        self.baseline_path = "baseline_data/comparison_data.npz"
...
def visualize_results(sbeed_steps, sbeed_rets, args):
    ...
    if args.viz_mode == "compare":
        if os.path.exists(args.baseline_path):
            try:
                base_data = np.load(args.baseline_path, allow_pickle=True)
...
def main(args):
    ...
    try:
        ...
        print(f"[Info] Loaded data from {args.data_path}")
    except Exception as e:
    ...
    try:
        ...
        print(f"[Error] .npz file is missing key: {e}")
        return
